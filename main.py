...
def check_binance_status():
    ...
    try:
        response = requests.get(
            f"{settings['BASE_URL']}/api/v3/ping", timeout=5
        )
        return response.status_code == 200
    ...
    for attempt in range(retries):
        if check_internet_connection() and check_binance_status():
            try:
                ...
                logging.error(
                    f"SSL Error: {ssl_error}. Coba lagi dalam {delay} detik..."
                )
                await asyncio.sleep(delay)
            except requests.exceptions.ConnectionError as conn_error:
                logging.error(
                    f"Connection Error: {conn_error}. Coba lagi dalam {delay} detik..."
                )
                await asyncio.sleep(delay)
            except Exception as e:
                logging.error(
                    f"Error saat melakukan request: {e}. Coba lagi dalam {delay} detik..."
                )
                await asyncio.sleep(delay)
        else:
    ...
    def on_modified(self, event):
        ...
        try:
            ...
            kirim_notifikasi_telegram(
                f"Error saat memuat ulang bot: {e}"
...
async def main():
    ...
    # Gunakan pathlib untuk mengelola path
    import pathlib
    ...
    try:
        ...
        event_handler = ReloadHandler(
            bot
        )  # Membuat handler untuk perubahan file
        ...
        kirim_notifikasi_telegram(
            f"Error saat menjalankan bot: {e}"
        )  # Kirim pesan error ke Telegram
    finally:
...
if __name__ == "__main__":
    try:
        ...
        logging.critical(
            f"Terjadi kesalahan fatal saat menjalankan aplikasi: {e}"
        )
        kirim_notifikasi_telegram(
            f"Terjadi kesalahan fatal saat menjalankan aplikasi: {e}"
        )
