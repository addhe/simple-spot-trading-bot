...
class BotTrading:
    def __init__(self):
        ...
        self.latest_activities = {
            symbol: self.storage.load_latest_activity(symbol) for symbol in SYMBOLS
        }
    ...
    def get_config_hash(self):
        ...
        try:
            config_str = (
                f"{settings['API_KEY']}{settings['API_SECRET']}{str(SYMBOLS)}{INTERVAL}"
            )
        ...
        try:
            ...
            for symbol_info in exchange_info["symbols"]:
                if symbol_info["symbol"] in SYMBOLS:
                    self.symbol_info[symbol_info["symbol"]] = self.extract_symbol_info(
                        symbol_info
                    )
                    logging.info(
                        f"Initialized {symbol_info['symbol']} info: {self.symbol_info[symbol_info['symbol']]}"
                    )
        except Exception as e:
        ...
        try:
            ...
            if lot_size_filter:
                symbol_specific_info.update(
                    {
                        "quantity_precision": self.get_precision_from_step_size(
            ...
            if price_filter:
                symbol_specific_info["price_precision"] = (
                    self.get_precision_from_step_size(price_filter["tickSize"])
                )

            if min_notional_filter:
                symbol_specific_info["min_notional"] = float(
            ...
            logging.warning(
                f"Error processing filters for {symbol_info['symbol']}, using default values: {str(e)}"
            )
            return symbol_specific_info
    ...
    def get_precision_from_step_size(self, step_size: str) -> int:
        try:
            ...
            logging.error(
                f"Error calculating precision from step size {step_size}: {str(e)}"
            )
            return 8
    ...
    def calculate_dynamic_quantity(self, symbol: str, price: float) -> float:
        try:
            ...
            quantity = max(
                symbol_info["min_quantity"], min(quantity, symbol_info["max_quantity"])
            )

            if quantity * price < symbol_info["min_notional"]:
                logging.warning(
                    f"Order value below minimum notional ({symbol_info['min_notional']})"
                )
                return 0.0
    ...
    def update_symbol_usdt_allocation(self):
        ...
        try:
            ...
            self.symbol_usdt_allocation = {
                symbol: allocation_per_symbol for symbol in SYMBOLS
            }
            logging.info(
                f"Updated USDT allocation per symbol: {self.symbol_usdt_allocation}"
            )
        except Exception as e:
    ...
    def get_all_asset_status(self) -> dict:
        try:
            asset_status = {}
            for symbol in SYMBOLS:
                asset_info = self.client.get_asset_balance(
    ...
    async def check_prices(self):
        for symbol in SYMBOLS:
            try:
                ...
                if (
                    action == "BUY"
                    and not latest_activity["buy"]
                    and not self.has_active_orders(symbol, "BUY")
                ):
                ...
                # Cek jika action adalah SELL dan sudah ada pembelian sebelumnya
                    ...
                    action == "SELL"
                    and latest_activity["buy"]
                    and not self.has_active_orders(symbol, "SELL")
                ):
                    await self.execute_sell(symbol, price, latest_activity)

                # Cek jika harus menjual berdasarkan strategi
                if latest_activity["buy"] and strategy.should_sell(
                    price, latest_activity
                ):
                    await self.execute_sell(symbol, price, latest_activity)
    ...
    async def execute_buy(
        self, symbol: str, price: float, quantity: float, strategy: PriceActionStrategy
    ):
        try:
            rounded_price = round(price, self.symbol_info[symbol]["price_precision"])
            rounded_quantity = round(
                quantity, self.symbol_info[symbol]["quantity_precision"]
            )
    ...
    async def execute_sell(self, symbol: str, price: float, activity):
        try:
            ...
            asset_status = self.get_all_asset_status()
            notifikasi_sell(
                symbol, activity["quantity"], price, usdt_balance, asset_status
            )
        except (BinanceAPIException, ConnectionError, Timeout) as e:
